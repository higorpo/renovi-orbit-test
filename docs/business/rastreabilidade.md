...
| `settings/` | `api/*Profile*.api.ts`, `portfolio.api.ts`, `offeredServices.api.ts`; `constants/routes.ts`, `settingsNav.ts` (`SHARED_TAIL`: privacy → legal → session); `TERMS_OF_USE_URL` / `PRIVACY_POLICY_URL` / `PROVIDER_PLATFORM_CONTRACT_URL` | Hub `SettingsLayout` / `SettingsIndexPage` / `sections/*` (incl. `AccountLegalPage`, `AccountPrivacyPage`, `AccountSessionPage`, `ProviderProfessionalProfilePage` com Tabs Pedidos/Vitrine); `LegalDocumentsSection`, `PrivacySection`, `AccountSummaryCard`, `OfferedServicesSection`, `PublicProfileSettingsSection`, `ServiceAreaSection`/`ServiceAreaField`, `PortfolioManagementSection`, `ProfessionalProfileFormSkeleton` |
