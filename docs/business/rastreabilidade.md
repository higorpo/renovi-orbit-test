...
| `settings/` | `api/*Profile*.api.ts` (`getProviderPrivateProfile` `select *`; `UpdateProviderPrivateParams` só identidade legal), `portfolio.api.ts`, `offeredServices.api.ts`; `constants/routes.ts` (`ROUTE_SETTINGS_PAYOUT_METHODS`, `ROUTE_SETTINGS_RECEIVABLES` = earnings `?view=charges`), `settingsNav.ts` (prestador: Dados bancários `Landmark` depois de Perfil profissional e antes de Ganhos; sem Recebimentos; `PROVIDER_ONLY_SETTINGS_SECTIONS` inclui `payoutMethods`; `SHARED_TAIL`: privacy → legal → session); `hooks/useEarningsLedgerSummary.ts` (range `received_*` / `settling_*`); `hooks/useProviderPayoutMethods.ts`; `TERMS_OF_USE_URL` / `PRIVACY_POLICY_URL` / `PROVIDER_PLATFORM_CONTRACT_URL` | Hub `SettingsLayout` / `SettingsIndexPage` / `sections/*` (incl. `AccountLegalPage`, `AccountPrivacyPage`, `AccountSessionPage`, `ProviderProfessionalProfilePage` com Tabs Pedidos/Vitrine, `ProviderPayoutMethodsPage` + `PayoutMethodsSection` somente leitura, `ProviderEarningsSectionPage` unificado com chips de período, `ProviderReceivablesPage` redirect); `LegalDocumentsSection`, `PrivacySection`, `AccountSummaryCard`, `OfferedServicesSection`, `PublicProfileSettingsSection`, `ServiceAreaSection`/`ServiceAreaField`, `PortfolioManagementSection`, `ProfessionalProfileFormSkeleton`, `PayoutMethodsFormSkeleton` |
...
| `provider-kyc/` | `api/kyc.api.ts`, `providerKyc.rpc.ts`, `brazilianBanks.api.ts`; hooks `useProviderPaymentAccount`, `useProviderKycBlocksNav`, `useProviderKycWizard`, `useDispatchKyc`, `useBrazilianBanks`; constantes `PROVIDER_KYC_SUPPORT_URL` / `PROVIDER_KYC_HELP_MAILTO`, `formatBankLabel` | `ProviderKycGate`, `ProviderKycForm`, `BankPicker`, `ProviderKycWizardStepContent`, telas `components/status/*`; catálogo/URLs também consumidos por settings Dados bancários (somente leitura) |
| `auth/` | (já listado) | — |
