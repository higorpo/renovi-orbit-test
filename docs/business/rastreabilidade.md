...
| `settings/` | `api/*Profile*.api.ts`, `portfolio.api.ts`, `offeredServices.api.ts`; `constants/routes.ts` (`ROUTE_SETTINGS_RECEIVABLES` = earnings `?view=charges`), `settingsNav.ts` (prestador: um item Ganhos, sem Recebimentos; `SHARED_TAIL`: privacy → legal → session); `hooks/useEarningsLedgerSummary.ts` (range `received_*` / `settling_*`); `TERMS_OF_USE_URL` / `PRIVACY_POLICY_URL` / `PROVIDER_PLATFORM_CONTRACT_URL` | Hub `SettingsLayout` / `SettingsIndexPage` / `sections/*` (incl. `AccountLegalPage`, `AccountPrivacyPage`, `AccountSessionPage`, `ProviderProfessionalProfilePage` com Tabs Pedidos/Vitrine, `ProviderEarningsSectionPage` unificado com chips de período, `ProviderReceivablesPage` redirect); `LegalDocumentsSection`, `PrivacySection`, `AccountSummaryCard`, `OfferedServicesSection`, `PublicProfileSettingsSection`, `ServiceAreaSection`/`ServiceAreaField`, `PortfolioManagementSection`, `ProfessionalProfileFormSkeleton` |
