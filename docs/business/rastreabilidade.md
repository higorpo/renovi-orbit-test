...
| `settings/` | `api/*Profile*.api.ts` (`getProviderPrivateProfile` `select *`; `UpdateProviderPrivateParams` só identidade legal), `portfolio.api.ts`, `offeredServices.api.ts`; `constants/routes.ts` (`ROUTE_SETTINGS_KYC_DOCUMENTS`, `ROUTE_SETTINGS_PAYOUT_METHODS`, `ROUTE_SETTINGS_RECEIVABLES` = earnings `?view=charges`), `settingsNav.ts` (prestador: Documentos `FileText` depois de Identidade legal e antes de Perfil profissional; Dados bancários `Landmark` depois de Perfil profissional e antes de Ganhos; sem Recebimentos; `PROVIDER_ONLY_SETTINGS_SECTIONS` inclui `kycDocuments` e `payoutMethods`; `SHARED_TAIL`: privacy → legal → session); `hooks/useEarningsLedgerSummary.ts` (range `received_*` / `settling_*`); `hooks/useProviderPayoutMethods.ts`; `hooks/useProviderKycDocuments.ts`; `TERMS_OF_USE_URL` / `PRIVACY_POLICY_URL` / `PROVIDER_PLATFORM_CONTRACT_URL` | Hub `SettingsLayout` / `SettingsIndexPage` / `sections/*` (incl. `AccountLegalPage`, `AccountPrivacyPage`, `AccountSessionPage`, `ProviderProfessionalProfilePage` com Tabs Pedidos/Vitrine, `ProviderKycDocumentsPage` + `KycDocumentsSection` somente leitura + download, `ProviderPayoutMethodsPage` + `PayoutMethodsSection` somente leitura, `ProviderEarningsSectionPage` unificado com chips de período, `ProviderReceivablesPage` redirect); `LegalDocumentsSection`, `PrivacySection`, `AccountSummaryCard`, `OfferedServicesSection`, `PublicProfileSettingsSection`, `ServiceAreaSection`/`ServiceAreaField`, `PortfolioManagementSection`, `ProfessionalProfileFormSkeleton`, `KycDocumentsFormSkeleton`, `PayoutMethodsFormSkeleton` |
...
| `payments/` | APIs checkout/cartões/histórico/cobrança; RPCs `payment_*`; `summarizeProviderReceivables`; `listProviderPaymentReceivables` (`receivedFrom`/`receivedTo` em `received_at`) | Checkout stepper, `ManualPaymentDialog`, histórico cliente em Pagamentos; captura prestador em Ganhos → Cobranças (filtro de período da página host) |
| `provider-earnings/` | `api/settlements.api.ts`; `useProviderSettlements` (envia `p_settling_from`/`p_settling_to`); `useEarningsViewParam` / `parseEarningsView` / `parseEarningsPeriod` / `getEarningsPeriodRange` / `providerEarningsPath`; `earningsPeriod.ts`; `earningsPeriodRange.ts`; disclosure D+30 / `settling_at`; `ROUTE_PROVIDER_EARNINGS` | `EarningsLedgerSwitch` (padrão Tabs Pagamentos, `aria-label="Listas de ganhos"`, chips de período) + `EarningsPage` (host em `settings` `/dashboard/settings/earnings`); filtros Todos/Previsto/Liquidado (CREDIT); `ProviderSettlementDisclosure` |
| `provider-kyc/` | `api/kyc.api.ts` (`getKycDocumentSignedUrl`, upload Option A, submit), `providerKyc.rpc.ts`, `brazilianBanks.api.ts`; `utils/kycOnboardingDocuments.ts` (`listKycOnboardingDocuments`); hooks `useProviderPaymentAccount`, `useProviderKycBlocksNav`, `useProviderKycWizard`, `useDispatchKyc`, `useBrazilianBanks`; constantes `PROVIDER_KYC_SUPPORT_URL` / `PROVIDER_KYC_HELP_MAILTO`, `KYC_DOCUMENT_SIGNED_URL_EXPIRY_SEC`, `formatBankLabel` | `ProviderKycGate`, `ProviderKycForm`, `BankPicker`, `ProviderKycWizardStepContent`, telas `components/status/*`; catálogo/URLs/slots/signed URL também consumidos por settings Dados bancários e Documentos (somente leitura) |
| `auth/` | (já listado) | — |
...
| `src/features/payments/components/PaymentHistory/*` | UI histórico cliente (Pagamentos) / prestador (Ganhos → Cobranças); lista prestador sem disclosure por card |
| `src/features/payments/utils/summarizeProviderReceivables.ts` | Totais do ledger Cobranças (soma client-side de `amountReceivedAtCapture`) |
...
| `src/features/provider-earnings/` | UI Ganhos unificada (chips de período + ledger Tabs + depósitos) + Vitest (api/hooks/utils/disclosure/`earningsView`/`earningsPeriod`) |
...
| `src/features/provider-kyc/api/kyc.api.ts` | Critérios de status; upload Option A; submit com identidade; prefill `provider_profiles_private`; `getKycDocumentSignedUrl` (bucket `provider-kyc-documents`, 7 dias; path vazio → “Documento não encontrado”) |
| `src/features/provider-kyc/utils/kycOnboardingDocuments.ts` | `listKycOnboardingDocuments` — slots PF/PJ para Configurações → Documentos; PJ sem identity duplicado; fallback legal_rep → identity; `entity_type` ausente = PF |
| `src/features/settings/components/sections/ProviderKycDocumentsPage.tsx` | Hub Configurações → Documentos (`SettingsRoleGate` provider; header + `KycDocumentsSection`) |
| `src/features/settings/hooks/useProviderKycDocuments.ts` | Lista slots + download (`getKycDocumentSignedUrl` + `window.open`); toast de erro de download |
| `supabase/migrations/20260801185000_create_provider_kyc_documents_storage.sql` | Bucket `provider-kyc-documents`; SELECT do dono (signed URL sem RPC nova) |
