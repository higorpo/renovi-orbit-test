...
| `service-completion/` | `api/lifecycle.api.ts`, `context.api.ts`, `draft.api.ts`, `upload.api.ts`, `evidencePhotoStorage.api.ts`, `ratings.api.ts`, `declaration.api.ts`, `pendingEvaluationPrompt.api.ts`; hooks draft/mark/confirm/declaration/dispute/enrichment/evidence URLs / pending eval | **`ProviderMarkExecutedAction`**, **`ClientEvaluateServiceAction`**, **`PendingEvaluationPromptHost`**, `CompletionFlowSheetDialog`, `CompletionEvidenceGallery`, wizards embutidos (checkbox Declaração de execução no path manual), `DisputeStubEntry` (Public API → `view-services` / `RootLayout`) |
...
| `docs/business/modulos/service-completion/` | Enrichment, conclusão EXECUTED (manual ou auto-mark sem checklist) / confirm / auto-complete, **Declaração de execução** (gate path manual; ausente no auto-complete), stub disputa; CTAs sheet/dialog + galeria evidências; **prompt de avaliação pendente** (RPC leve + host no RootLayout; fila após localização/push); endurecimento SQL (evidência registrada, contexto full vs marketplace, imutabilidade); lazy load de `get_service_completion_context` (detalhe via campos leves de `get_service`); SELECT storage `completion-evidence` também para cliente com evidência `frozen` (`createSignedUrl`) |
| `docs/service-completion/` | Design técnico / ADR (fonte normativa de engenharia); ADR-0005 declaração de execução; `storage-bucket.md` (políticas SELECT/INSERT) |
