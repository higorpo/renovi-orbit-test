...
| `payments/` | APIs checkout/cartões/histórico/cobrança; RPCs `payment_*`; `summarizeProviderReceivables` | Checkout stepper, `ManualPaymentDialog`, histórico cliente em Pagamentos; captura prestador em Ganhos → Cobranças |
| `provider-earnings/` | `api/settlements.api.ts`; `useProviderSettlements`; `useEarningsViewParam` / `parseEarningsView` / `providerEarningsPath`; disclosure D+30 / `settling_at`; `ROUTE_PROVIDER_EARNINGS` | `EarningsLedgerSwitch` + `EarningsPage` (host em `settings` `/dashboard/settings/earnings`); filtros Todos/Previsto/Liquidado (CREDIT); `ProviderSettlementDisclosure` |
...
| `src/features/provider-earnings/` | UI Ganhos unificada (ledger + depósitos) + Vitest (api/hooks/utils/disclosure/`earningsView`) |
