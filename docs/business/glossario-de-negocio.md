...
| **Reembolso solicitado (`REFUND_REQUESTED`)** | Estado após pedido de estorno: parcela já tem `refunded_amount` **esperado**; `refunded_at` só após confirmação do gateway (webhook/reconciliação). Retry da Edge pode reenviar ao gateway até ACK (`refund_submit_status`). | RPC `payment_begin_refund_request`; Edge `process-refund`. |
