...
| **Fluxo de disputa completo na UI** | Stub em **service-completion** (`DisputeStubEntry`): banner “Abrir disputa” (descrição sobre correção pelo prestador ou devolução parcial/integral); URL de suporte via `VITE_SERVICE_COMPLETION_DISPUTE_SUPPORT_URL` / `orbit.dispute_support_url`, ou toast “Em breve” + analytics — **sem** FSM de disputa. Separado: flag `is_disputed` / badge de chargeback no detalhe (payments) e webhook de dispute no gateway. |
