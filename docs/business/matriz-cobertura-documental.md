...
| my-services | Shell compartilhado; slot por role; abas/filtros RPC; deep link; sheet compare/history; card pipeline; highlight `PENDING_PAYMENT` / `FAILED_PERMANENT`; follow-up conclusão pós-data-fim / `EXECUTED`; prestador “Concluir serviço” no card; nota real no card `completed` (`clientRatingOverallScore`) | OK (`solicitacoes-do-cliente.md`, README §8–9) | Sim (23) | Dropdowns só da página carregada; aba Disputas vazia | **OK** |
| view-services | RPCs unificados; `ServiceModel`; detalhe real por fase (`ServiceDetailShell`); conclusão via **service-completion** (CTAs → sheet/dialog) | OK (`visualizacao-de-servicos.md`) | Sim (24) | pgTAP `view_services_rpcs_test.sql`; aba Disputas vazia | **OK** |
...
| service-completion | Enrichment READY-handoff; checklist; upload evidência Option A; mark EXECUTED; auto-mark EXECUTED sem checklist; confirm+rating (CTA sheet 2 etapas; prompt global 3 etapas); auto-complete ~24h após `executed_at`; stub disputa; galeria thumbnails+lightbox (`createSignedUrl` cliente com evidência frozen); contexto full vs marketplace; janitor SQL órfãos; RPC `get_client_pending_evaluation_prompt` | OK (`conclusao-e-enrichment.md`) | Sim (20) | FSM disputa completa = stub only (SC-01); aba Disputas vazia (SC-02) | **OK** |
...
| provider-kyc | Gate de **conteúdo** até `ACTIVE` + **ocultação do chrome** de nav; wizard Fase 3; BankPicker FEBRABAN; upload Option A; e-mail ops | OK (`gate-e-acesso-operacional.md`, `formulario-credenciamento-wizard.md`) | Sim (20) | — | **OK** |
