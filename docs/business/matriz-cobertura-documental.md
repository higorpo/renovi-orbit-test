...
**Atualização 2026-08-12 (Documentos KYC):** seção só prestador `/dashboard/settings/kyc-documents` — consulta/download dos anexos do onboarding (`listKycOnboardingDocuments` + `getKycDocumentSignedUrl`); CTA suporte; sem auto-save, sem re-upload, sem RPC/migration (Storage SELECT do dono já existe). Distinto de Identidade legal, Jurídico e Dados bancários. Docs settings / provider-kyc / glossário / mapa / rastreabilidade / perfis.

**Atualização 2026-08-12 (Dados bancários):** seção só prestador `/dashboard/settings/payout-methods` — consulta somente leitura de `provider_profiles_private` (banco/agência/conta/PIX); CTA suporte via `provider-kyc`; sem auto-save, sem `BankPicker`, sem update de `bank_*`/`pix_key`, sem RPC/migration. Docs settings / provider-kyc / glossário / mapa / rastreabilidade.
...
| settings | Hub settings `/dashboard/settings/*`; Perfil profissional em Tabs Pedidos (ofertados+área) / Vitrine (público+portfólio); **Documentos** (`kyc-documents`, anexos KYC somente leitura + download); **Dados bancários** (`payout-methods`, somente leitura); portfólio; área; exclusão; seção Jurídico (`legal`); embute cartões + histórico cliente; host Ganhos unificado (Cobranças + Depósitos) | OK (`configuracoes.md` + cross-link payments/earnings/kyc) | Sim (21) | Impacto legal de exclusão de conta; deep_link SQL legado `/dashboard/conta` | **OK** |
...
| provider-kyc | Gate de **conteúdo** até `ACTIVE` + **ocultação do chrome** de nav; wizard Fase 3; BankPicker FEBRABAN; upload Option A; e-mail ops; **lembretes MMD** (`PENDING_DOCUMENTS`/`REJECTED`); dados bancários persistidos também **exibidos** (não editados) e anexos **consultados/baixados** em Configurações | OK (`gate-e-acesso-operacional.md`, `formulario-credenciamento-wizard.md`, `lembretes-credenciamento-incompleto.md`) | Sim (20) | — | **OK** |
