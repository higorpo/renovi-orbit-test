...
**Atualização 2026-08-12:** hub Configurações `/dashboard/settings/*` (fase 1 shell); Ganhos hospedado em `/dashboard/settings/earnings`; menu sem Endereços/Ganhos; allowlist KYC `/dashboard/settings`; P-04 fechada.
...
| settings | Hub settings `/dashboard/settings/*`; portfólio; área; exclusão; seção Jurídico (`legal`); embute cartões + histórico; host Ganhos | OK (`configuracoes.md` + cross-link payments/earnings) | Sim (21) | Impacto legal de exclusão de conta; deep_link SQL legado `/dashboard/conta` | **OK** |
