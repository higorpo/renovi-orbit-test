...
**Atualização 2026-08-12:** hub Minha conta `/dashboard/account/*` (fase 1 shell); Ganhos hospedado em `/dashboard/account/earnings`; menu sem Endereços/Ganhos; allowlist KYC `/dashboard/account`; P-04 fechada.
...
|--------|------------------------|--------------|------------------|-----------------------------|--------|
| addresses | CRUD endereços; seleção no wizard; CEP; geografia | OK (`gestao-de-enderecos.md`) | Sim (21) | Gestão real em `/dashboard/account/addresses` (+ wizard); rota menu `/dashboard/addresses` removida | **OK** |
...
| my-account | Hub settings `/dashboard/account/*`; portfólio; área; exclusão; embute cartões + histórico; host Ganhos | OK (`minha-conta.md` + cross-link payments/earnings) | Sim (21) | Impacto legal de exclusão de conta; deep_link SQL legado `/dashboard/conta` | **OK** |
...
| provider-earnings | Ganhos: liquidações `PENDING`/`PAID_OUT`/`DEBIT`; disclosure D+30; rota `/dashboard/account/earnings` (sem menu top-level) | OK (`ganhos-e-liquidacoes.md`) | Sim (20) | — | **OK** |
...
| Rotas placeholder identificadas | ≥3 (`/dashboard`, `/dashboard/settings`, `/dashboard/help`) — **não** inclui `/dashboard/services/:id` (real), `/dashboard/services/calendar` (real), nem hub `/dashboard/account/*` (real). Removidas: `/dashboard/addresses`, `/dashboard/earnings`, `/dashboard/conta` |
