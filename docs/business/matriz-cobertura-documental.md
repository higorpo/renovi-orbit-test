...
| view-services | RPCs unificados; `ServiceModel`; detalhe real por fase (`ServiceDetailShell`); **Jornada do pedido** (`get_client_service_journey` + `ServiceJourneyCard`, cliente-only); conclusão via **service-completion** (CTAs → sheet/dialog); `list_phase=dispute` | OK (`visualizacao-de-servicos.md`) | Sim (24) | pgTAP `view_services_rpcs_test.sql`, `get_client_service_journey_test.sql` | **OK** |
