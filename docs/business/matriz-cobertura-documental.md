...
| provider-earnings | Ganhos unificado: ledger Cobranças (soma captura) + Depósitos (contagem CREDIT `PENDING`/`PAID_OUT`); disclosure D+30; rota `/dashboard/settings/earnings` (sem menu top-level; sem item Recebimentos na nav) | OK (`ganhos-e-liquidacoes.md`) | Sim (20) | — | **OK** |
