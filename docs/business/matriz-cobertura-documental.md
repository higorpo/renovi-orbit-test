...
| view-services | RPCs unificados; `ServiceModel`; detalhe real por fase (`ServiceDetailShell`); conclusão via **service-completion** (wizards) | OK (`visualizacao-de-servicos.md`) | Sim (24) | pgTAP `view_services_rpcs_test.sql`; aba Disputas vazia | **OK** |
...
| service-completion | Enrichment READY-handoff; checklist; upload evidência Option A (RPC + storage autenticado, sem Edge URL); mark EXECUTED (paths registrados); confirm+rating; auto-complete ~24h (`auto_complete_batch_size`); stub disputa; contexto full vs marketplace; janitor SQL órfãos (sem Edge) | OK (`conclusao-e-enrichment.md`) | Sim (20) | FSM disputa completa = stub only (SC-01); aba Disputas vazia (SC-02) | **OK** |
