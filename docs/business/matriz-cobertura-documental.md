...
| provider-kyc | Gate de **conteúdo** até `ACTIVE` + **ocultação do chrome** de nav; wizard Fase 3; BankPicker FEBRABAN; upload Option A; e-mail ops; **lembretes MMD** (`PENDING_DOCUMENTS`/`REJECTED`); dados bancários persistidos também **exibidos** (não editados) em Configurações | OK (`gate-e-acesso-operacional.md`, `formulario-credenciamento-wizard.md`, `lembretes-credenciamento-incompleto.md`) | Sim (20) | — | **OK** |
