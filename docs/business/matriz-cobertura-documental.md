...
| settings | Hub settings `/dashboard/settings/*`; Perfil profissional em Tabs Pedidos (ofertados+área) / Vitrine (público+portfólio); portfólio; área; exclusão; seção Jurídico (`legal`); embute cartões + histórico; host Ganhos | OK (`configuracoes.md` + cross-link payments/earnings) | Sim (21) | Impacto legal de exclusão de conta; deep_link SQL legado `/dashboard/conta` | **OK** |
