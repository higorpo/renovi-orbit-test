...
**Atualização 2026-08-12:** hub Configurações `/dashboard/settings/*` (fase 1 shell); Ganhos hospedado em `/dashboard/settings/earnings`; menu sem Endereços/Ganhos; allowlist KYC `/dashboard/settings`; P-04 fechada. **Atualização 2026-08-12 (Perfil profissional UI):** quatro cards em `/dashboard/settings/professional-profile` (ofertados, público, área, portfólio); skeleton dedicado; sem mudança de API — docs settings/glossário/rastreabilidade.
...
| settings | Hub settings `/dashboard/settings/*`; Perfil profissional em 4 cards (ofertados/público/área/portfólio); portfólio; área; exclusão; seção Jurídico (`legal`); embute cartões + histórico; host Ganhos | OK (`configuracoes.md` + cross-link payments/earnings) | Sim (21) | Impacto legal de exclusão de conta; deep_link SQL legado `/dashboard/conta` | **OK** |
...
| provider-earnings | Ganhos: liquidações `PENDING`/`PAID_OUT`/`DEBIT`; disclosure D+30; rota `/dashboard/settings/earnings` (sem menu top-level) | OK (`ganhos-e-liquidacoes.md`) | Sim (20) | — | **OK** |
