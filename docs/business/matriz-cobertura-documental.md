...
| payments | Checkout/T-2/KYC; `charge_amount` = gross-up NetCred `(base + PROCESSING + RISK_ANALYSIS) / (1 − MDR%)` (`cc_risk_analysis_fee_brl`; antecipação fora; split prestador `FIXED_AMOUNT` inalterado); tokenização coleta CPF do titular (enviado à NetCred, independente do CPF da conta); comparação do primeiro nome do cartão com o perfil apenas alerta e não bloqueia; histórico cliente (breakdown com `refunded_amount`); `REFUND_REQUESTED` persiste valor esperado sem `refunded_at`; recebimentos prestador com clawback só após `refunded_at`; erros de checkout/cartão/cobrança manual → mensagens amigáveis pt-BR; rejeição ClearSale “Análise de Risco” → `RISK_ANALYSIS_*` em `failure_code` (bruto em `failure_reason`); recuperação manual via `ManualPaymentDialog` + `ManualPaymentFailureAlert` | OK (`checkout-e-cobranca.md`, `historico-e-reembolso.md`) | Matriz completa de faixas de multa/cancelamento ToS (detalhe em `docs/payment-system/`); catálogo exaustivo de *todos* os códigos de pagamento→mensagem na doc de negócio (códigos `RISK_ANALYSIS_*` já documentados) | — |
