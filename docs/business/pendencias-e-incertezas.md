...
| P-04 | Menu vs rota “Endereços” | ~~Menu apontava `/dashboard/addresses` (fake).~~ **Fechada (2026-08-12):** item Endereços e rota removidos; gestão em `/dashboard/settings/addresses`. | — | **Fechada** |
...
- “Visão geral” no menu é **placeholder** até nova implementação. Item/rota **Ajuda** (`/dashboard/help`) **removidos** (sem redirect). **Configurações** é hub real (`settings`, `/dashboard/settings/*`). **Ganhos** é feature real (`provider-earnings`) em `/dashboard/settings/earnings` (sem item de menu) — ver [ganhos-e-liquidacoes](./modulos/provider-earnings/features/ganhos-e-liquidacoes.md). **Calendário** (`/dashboard/services/calendar`) é feature real (`provider-calendar`); entrada via banner em Meus Serviços (prestador), sem item de menu dedicado (PC-02).
- ~~Prestador compartilha o menu operacional completo independentemente do KYC.~~ ~~**Corrigido (2026-07-30):** sem onboarding `ACTIVE`, o menu do prestador fica só em Configurações e o shell operacional é bloqueado pelo `ProviderKycGate`.~~ ~~**Atualizado (2026-08-03):** menu completo do prestador **sempre** visível (`getDashboardMenu`); o `ProviderKycGate` continua substituindo o **conteúdo** operacional até `ACTIVE`.~~ **Atualizado (2026-08-03):** menus **completamente ocultos** enquanto loading ou KYC ≠ `ACTIVE` (`useProviderKycBlocksNav` + `DashboardLayout`); gate continua substituindo o **conteúdo**; allowlist `/dashboard/settings*`; header/logo permanece — ver [provider-kyc](./modulos/provider-kyc/features/gate-e-acesso-operacional.md). Hook `useProviderKycNavItems` removido anteriormente.
