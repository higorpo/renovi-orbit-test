...
| **Abertas (conclusão / disputa stub)** | SC-01 (FSM disputa completa fora do escopo), SC-02 (aba Disputas vazia) — documentados; não bloqueiam comportamento shipped. Endurecimento SQL 2026-08-05 (evidência/sessões/contexto/RLS; janitor órfãos SQL-only, sem Edge) **documentado**; upload evidência Option A **documentado**; UX CTAs sheet/dialog + galeria evidências (2026-08-05) **documentada**; lazy load do completion context (2026-08-06) **documentado**; highlight de follow-up nos cards Meus Serviços pós-data-fim/`EXECUTED` (2026-08-06) **documentado** — sem nova pendência. |
