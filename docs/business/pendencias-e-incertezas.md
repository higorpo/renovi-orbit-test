...
- **Mensagens de erro do servidor:** RPCs retornam JSON estruturado; nem todas as chaves foram catalogadas nas features. **Exceção de UX (pagamentos):** na feature `payments`, a UI mapeia códigos para mensagens amigáveis em pt-BR e **não** exibe texto bruto do backend (checkout, cartões, cobrança manual) — ver [checkout-e-cobranca](./modulos/payments/features/checkout-e-cobranca.md#mensagens-de-erro-na-ui-pt-br). O catálogo completo código→cópia na documentação de negócio ainda não é exaustivo.
