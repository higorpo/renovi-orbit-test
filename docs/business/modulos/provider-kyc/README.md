...
- **Valor:** só prestadores credenciados operam na plataforma; header com **logo** permanece; allowlist de conteúdo **`/dashboard/conta*`** intacta. Menus ficam **completamente ocultos** enquanto loading ou status ≠ `ACTIVE`.
...
- **Exceção de path:** `/dashboard/conta` e `/dashboard/conta/…` liberam children **depois** do loading (allowlist `PROVIDER_KYC_ALLOWED_PATH_PREFIX`); chrome de nav **permanece oculto** se ainda não-`ACTIVE`.
...
| Lembretes de credenciamento incompleto | Cron diário (11:00 UTC) + MMD push/e-mail para `PENDING_DOCUMENTS` / `REJECTED`; deep link `/dashboard/conta` | [features/lembretes-credenciamento-incompleto.md](./features/lembretes-credenciamento-incompleto.md) |
