...
- **Para que serve:** checkout com cartão (tokenização NetCred), cobrança automática T-2 antes do serviço, cobrança manual pelo cliente, histórico de pagamentos/recebimentos (com breakdown de reembolso), **requisito de KYC `ACTIVE` para cobrança** e notificações de cobrança.
- **Quem usa:** cliente (checkout, cartões salvos, histórico com reembolsos, cobrança manual); prestador (histórico de recebimentos líquidos; **UI/gate de KYC** em `provider-kyc`); operação (runbooks em `docs/payment-system/`).
