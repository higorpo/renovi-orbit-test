...
- **`service-completion`:** writers de produto `EXECUTED`/`COMPLETED` (`service_completion_*`) — **fora** deste módulo (ADR-0004). Removidos: `payment_mark_service_executed`, `payment_confirm_service_completed`, `payment_cron_auto_complete_*`. Chargeback/`is_disputed` permanece aqui; stub de disputa in-app é do service-completion.
