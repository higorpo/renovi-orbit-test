...
- **Para que serve:** checkout com cartão (tokenização NetCred), cobrança automática T-2 antes do serviço, cobrança manual pelo cliente, histórico de pagamentos/recebimentos **na captura** (com breakdown de reembolso), **requisito de KYC `ACTIVE` para cobrança** e notificações de cobrança. Ops: auto-cancel T-12h, void pós-`IN_ANALYSIS`, reconciliação stale e sync de liquidações. Liquidações bancárias do prestador (Ganhos) são UI em [`provider-earnings`](../provider-earnings/README.md); este módulo mantém a persistência (`payment_settlement_movements`, webhooks `PAYOUT_*`, cron `sync-netcred-settlements`).
