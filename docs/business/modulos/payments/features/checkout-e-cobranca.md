...
| **Cartões salvos** | `SavedCardsList` em `/dashboard/settings/payments` → aba **Formas de pagamento** (`ClientPaymentsPage`) | Tokenização contexto `profile`; lista Prestway (skeleton / empty dashed); sem card/título aninhado — header da página é “Pagamentos”. |
...
- `src/features/settings/components/sections/ClientPaymentsPage.tsx` (Tabs: Formas de pagamento → `SavedCardsList`; Histórico → `PaymentHistorySection`)
