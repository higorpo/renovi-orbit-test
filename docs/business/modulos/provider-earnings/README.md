...
- **Para que serve:** mostrar ao **prestador** as **liquidações bancárias** (depósitos previstos e efetivos na conta), distintas dos **recebimentos na captura** em Minha conta → Recebimentos.
- **Quem usa:** prestadores autenticados (`profiles.role === provider`); UI hospedada no hub Minha conta (`/dashboard/account/earnings`), com `AccountRoleGate` provider. Sem KYC `ACTIVE`, o `ProviderKycGate` substitui o conteúdo operacional e o chrome de nav fica oculto (entrada via hub Minha conta na allowlist `/dashboard/account`).
- **Não é:** histórico de captura/`provider_payout` (isso fica em `payments` → `/dashboard/account/receivables`). Também não altera o calendário NetCred — só **lê** movements já sincronizados. **Não** há item **Ganhos** no `dashboardMenu` (removido).
...
- **Escopo:** UI em `/dashboard/account/earnings` (`ROUTE_PROVIDER_EARNINGS`); ownership da feature permanece em `provider-earnings`; host `ProviderEarningsSectionPage` em `my-account`. Filtros server-side, agrupamento visual por parcela do mesmo `payment_schedule_id`, Public API do disclosure.
- **Limites:** sem mutações; sem filtros de data na UI (RPC aceita, front não envia); sem analytics GA/Sentry específicos na feature.
- **Relação:** backend de ingestão e persistência em **payments**; navegação via hub **my-account** + chrome **dashboard-shell** + **provider-kyc**. Rota top-level `/dashboard/earnings` **removida** (sem redirect).
...
| Papel | Acesso |
|-------|--------|
| Prestador (`ACTIVE` KYC) | Seção Ganhos no hub Minha conta + RPC lista os próprios movements |
| Prestador (KYC não `ACTIVE`) | Menus ocultos; gate do shell; hub `/dashboard/account*` na allowlist (conteúdo liberado) |
| Cliente / visitante | Sem seção earnings; `AccountRoleGate` provider na página host |
...
1. Prestador abre **Minha conta → Ganhos** (`/dashboard/account/earnings`) → `useProviderSettlements` → RPC `list_provider_settlement_movements` → lista / vazio / erro.
...
- **my-account / payments UI:** Ganhos e Recebimentos no mesmo hub (`/dashboard/account/earnings` ↔ `/dashboard/account/receivables`); disclosure importado da Public API.
...
- **provider-kyc / dashboard-shell:** gate do outlet + `useProviderKycBlocksNav` (chrome oculto até `ACTIVE`); **sem** item Ganhos em `getDashboardMenu` — entrada só pelo hub Minha conta.
...
- `src/features/my-account/components/sections/ProviderEarningsSectionPage.tsx` — host no hub
