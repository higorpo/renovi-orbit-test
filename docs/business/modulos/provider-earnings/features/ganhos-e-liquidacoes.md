...
- **Resultado esperado:** lista com valor líquido, status Previsto/Liquidado, previsão/data de liquidação, parcela e máscara de conta; abas de filtro; disclosure de previsão em outras superfícies.
...
| Prestador KYC ≠ `ACTIVE` (ou conta ainda carregando) | Menu só Minha conta; outlet sob `ProviderKycGate` | `useProviderKycNavItems`; `DashboardLayout` |
...
**Ações bloqueadas:** papel errado → redirect dos guards; prestador sem KYC ativo → sem atalho Ganhos no menu (gate do shell).
...
3. Filtros UI → RPC: Todos (sem filtro); Previsto → `movement_status = PENDING`; Liquidado → `PAID_OUT`; Estorno → `record_type = DEBIT`.
...
| Filtros | Abas: Todos / Previsto / Liquidado / Estorno (server-side) |
...
|------|------|--------------|-----------|------|
| Abrir Ganhos | Prestador | Guard + (menu se KYC ACTIVE) | Carrega lista | Guard / gate |
...
- `src/layouts/DashboardLayout/dashboardMenu.ts`, `mobileNavigation.config.ts`, `DashboardLayout.tsx` (KYC nav)
