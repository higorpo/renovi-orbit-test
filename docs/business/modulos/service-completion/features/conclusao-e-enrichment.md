...
| Submeter rating pós auto | Cliente | `COMPLETED` + `completed_by=system` + sem rating (`canSubmitOptionalRating`) | Rating opcional: mesmo CTA/sheet, abre direto nas notas (`ratingOnly`); após envio, fase `success` com mode `optional`; mesma invalidação de lista/detalhe em `useClientConfirmRating` |
