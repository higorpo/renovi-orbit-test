# Conclusão de serviço, enrichment e disputa de serviço
...
| CTAs no host | `ProviderMarkExecutedAction` / `ClientEvaluateServiceAction` na `ServiceDetailActionsBar` (ao lado de cancelar/reagendar) |
...
| Quem | Pode | Não pode |
|------|------|----------|
| **Cliente** (dono do SR) | Contexto completo via RPC; revisar evidência frozen; **SELECT storage / `createSignedUrl`** em paths do CS quando evidência está `frozen` (thumbnails/lightbox em “Avaliar serviço”); **declaração de execução** (checkbox → Edge/RPC; hard gate antes de avaliar); confirm+rating em `EXECUTED`; **abrir disputa de serviço** só em `EXECUTED` → `IN_DISPUTE`; rating opcional pós auto-complete **ou** pós resolve admin; **prompt global** se `EXECUTED` na grace | Marcar EXECUTED; abrir disputa após `COMPLETED` / em `IN_DISPUTE`; retirar disputa self-serve; SELECT direto em `service_request_enrichments` ou em `service_completion_execution_declarations`; SELECT storage de evidência ainda em `draft` |
| **Prestador contratado** | Contexto completo; draft + mark EXECUTED em `CONFIRMED`; upload sob sessão própria; ver CS `IN_DISPUTE` (badge/lista Disputas; chat aberto) | Confirmar COMPLETED manual; abrir/resolver disputa; SELECT direto em enrichments |
...
| **Admin** (plataforma) | Contexto completo (mesmo sem ser participante); **resolver disputa** via RPC `service_completion_admin_resolve_dispute` (`is_platform_admin` ou `service_role`) — **sem** UI admin | Mutações self-serve de produto via UI do app |
| **Sistema** | Enrichment READY + bootstrap; auto-mark EXECUTED (sem checklist); auto-complete EXECUTED→COMPLETED (**não** se `IN_DISPUTE`); janitor órfãos; repair ≤7 dias | — |
...
| Disputa aberta (`IN_DISPUTE`) | Bloqueia auto-complete, confirm-with-rating e cancel; chat aberto; CTAs de avaliar/cancelar ocultos; hold Ganhos `service_dispute` |
| Resolve admin | `service_completion_admin_resolve_dispute` → `COMPLETED` (`completed_by=admin`); MMD cliente+prestador; SYSTEM no chat; rating opcional depois |
...
10. **Disputa de serviço:** cliente em `EXECUTED` abre via `service_completion_open_dispute` (CAS → `IN_DISPUTE`; motivo opcional; SYSTEM chat; MMD só prestador). Capability de **abrir** só em `EXECUTED` (não após `COMPLETED` / `IN_DISPUTE`). Em `IN_DISPUTE`: sem confirm, sem auto-complete, sem cancel self-serve. Resolve só `service_completion_admin_resolve_dispute` → `COMPLETED` (`completed_by=admin`). **≠** chargeback/`is_disputed`. ADR-0006; runbook em [service-completion-monitoring](../../../../service-completion/service-completion-monitoring.md).
...
| Disputa de serviço | Entrada **somente** no fluxo **Avaliar serviço** enquanto CS está `EXECUTED` (capability de abrir disputa) — **nunca** após `COMPLETED` nem inline no detalhe / barra de ações. Confirmação + motivo **opcional** → RPC `service_completion_open_dispute` → `IN_DISPUTE`. Em `IN_DISPUTE`: badge “Em disputa”; sem CTAs avaliar/cancelar; aba Disputas lista o item. Resolve só admin/`service_role` (sem UI). **≠** chargeback. |
...
| `service_completion_confirm_with_rating` | Auth cliente; `EXECUTED` (rejeita se `IN_DISPUTE`); scores obrigatórios; evidência frozen (invariante deferred) |
| `service_completion_open_dispute` | Auth cliente dono do SR; CAS `EXECUTED`→`IN_DISPUTE`; motivo opcional; SYSTEM chat; MMD prestador |
| `service_completion_admin_resolve_dispute` | Admin / `service_role`; `IN_DISPUTE`→`COMPLETED` (`completed_by=admin`); SYSTEM chat; MMD ambos |
| `service_completion_auto_complete_executed` | service_role / cron; só `EXECUTED` (CAS exclui `IN_DISPUTE`); grace hours após `executed_at`; batch `auto_complete_batch_size` (default 100); `SKIP LOCKED` |
...
Também: `EXECUTED` → `IN_DISPUTE` (cliente) → `COMPLETED` (admin RPC, `completed_by=admin`). Em `IN_DISPUTE` não há cancel self-serve.
...
| MMD | `SERVICE_EXECUTED` / `SERVICE_COMPLETED` / `SERVICE_AUTO_COMPLETED` / `SERVICE_DISPUTE_OPENED` / `SERVICE_DISPUTE_RESOLVED` |
| view-services | Host: CTAs na `ServiceDetailActionsBar` (Public API; gate leve via `enrichmentReady` do modelo + contexto só no fluxo); `list_phase = dispute` quando CS `IN_DISPUTE` |
| my-services | Cards `in_progress`: highlight de follow-up (pós-data-fim `CONFIRMED` / `EXECUTED`); prestador `CONFIRMED` + past → CTA **“Concluir serviço”** no card (sheet; contexto ao abrir); cliente `EXECUTED` → CTA **“Avaliar serviço”** no card (`ClientEvaluateServiceSheet` hospedado na página; contexto RPC só ao abrir o wizard); cards `completed`: `COMPLETED` + sem `clientRatingOverallScore` → **“Avaliar serviço”** com `ratingOnly` (dados de `list_services`); aba **Disputas** lista CS `IN_DISPUTE`; demais → “Ver detalhes” — ver [solicitacoes-do-cliente](../../my-services/features/solicitacoes-do-cliente.md) Anexos D e F |
| RootLayout / push-permission | `PendingEvaluationPromptHost` após fila localização + push (`appOpenOverlaySequence`) |
| Analytics | eventos de disputa aberta (sucesso/falha); `pending_evaluation_prompt_opened` / `_dismissed` / `_completed` |
| provider-earnings | Hold `service_dispute` enquanto CS `IN_DISPUTE` (copy ≠ chargeback) |
...
| Marcar executado (submit no sheet) | Prestador contratado | `CONFIRMED` + paths registrados + validação | `EXECUTED`; sessões → committed; sheet permanece aberta na fase `success` (`ProviderExecutedSuccessStep` → `CompletionSuccessStep`, `chrome="immersive"`); dismiss só com **“Entendi”** (ou fechar o shell); `useProviderMarkExecuted` invalida contexto + lista/detalhe (`SERVICES_LIST_QUERY_KEY` / `SERVICE_DETAIL_QUERY_KEY`) |
...
| Abrir “Avaliar serviço” | Cliente | UI: contrato `EXECUTED` (`canConfirmWithRating`) ou `COMPLETED` system/admin sem rating (`canSubmitOptionalRating`) | Manual: sheet/dialog 2 etapas (revisão + declaração + disputa se `EXECUTED` → rating). Opcional: sheet/dialog **só** etapa de notas (`ratingOnly`; sem disputa). Detalhe via `ClientEvaluateServiceAction` (mantém sheet montada enquanto `open` pós-capabilities); lista Meus Serviços via `ClientEvaluateServiceSheet` |
...
| Confirmar + avaliar | Cliente | `EXECUTED` | `COMPLETED` + rating; sheet permanece na fase `success` (`ClientEvaluateSuccessStep` mode `confirm` → `CompletionSuccessStep`, `chrome="immersive"`; CTA **“Entendi”**); sem toast de sucesso no hook; `useClientConfirmRating` invalida contexto + lista/detalhe (`SERVICES_LIST_QUERY_KEY` / `SERVICE_DETAIL_QUERY_KEY`) em qualquer host; hosts (`useClientEvaluateServiceDialog` / `usePendingEvaluationPrompt`) **não** fecham no `onCompleted` (prompt ainda invalida a query do pending); no prompt, o próximo pending só após dismiss da success (~800 ms) |
| Abrir disputa de serviço | Cliente | CS `EXECUTED` no fluxo Avaliar serviço | Confirmação + motivo opcional → `IN_DISPUTE`; notifica prestador; SYSTEM chat; fecha wizard de avaliação |
| Resolver disputa | Admin / `service_role` | CS `IN_DISPUTE` | `COMPLETED` (`completed_by=admin`); notifica ambos; SYSTEM chat; sem rating na mesma TX — ver runbook |
| Submeter rating pós auto / pós admin | Cliente | `COMPLETED` + (`completed_by=system` \| `admin`) + sem rating (`canSubmitOptionalRating`) | Rating opcional: mesmo CTA/sheet, abre direto nas notas (`ratingOnly`); após envio, fase `success` com mode `optional`; mesma invalidação de lista/detalhe em `useClientConfirmRating` |
...
- Após mark-executed com sucesso, `ProviderMarkExecutedSheet` troca fase `checklist` → `success` na mesma `CompletionFlowSheetDialog` (`chrome` standard → immersive; corpo `ProviderExecutedSuccessStep` → `CompletionSuccessStep`) e **não** fecha no `onExecuted` do host; toast de sucesso do mark foi removido do hook. A invalidação de Meus Serviços / detalhe (`SERVICES_LIST_QUERY_KEY` / `SERVICE_DETAIL_QUERY_KEY`) ocorre no `onSuccess` de `useProviderMarkExecuted` (não no wizard; chave legada `["services"]` removida — não batia com a lista real).
- Após avaliação com sucesso (confirm-with-rating ou optional rating), `ClientEvaluateServiceSheet` troca fase `wizard` → `success` na mesma `CompletionFlowSheetDialog` (`chrome` standard → immersive; corpo `ClientEvaluateSuccessStep` → `CompletionSuccessStep`; modes `confirm` \| `optional`) e **não** fecha no submit; toast de sucesso removido de `useClientConfirmRating`. A invalidação de Meus Serviços / detalhe ocorre no `onSuccess` de `useClientConfirmRating` (qualquer host: lista, prompt, detalhe). `ClientEvaluateServiceAction` mantém a sheet montada enquanto `open` após as capabilities sumirem.
...
- Chargeback / `is_disputed` em payments **não** é Disputa de serviço (`IN_DISPUTE`).
...
| Expectativa de inbox/reopen de disputa | MVP: sem painel admin; resolve via SQL/`service_role`; sem retirada self-serve |
...
- Migrations `20260804010000`–`2026080452*` (constants, RLS, evidence/sessions, mark/confirm/auto-complete, context RPC, janitor, indexes); `20260806180328_service_completion_auto_mark_executed.sql`; `20260806205555_get_client_pending_evaluation_prompt.sql`; `20260810170000_service_dispute_enum_audit_columns.sql`; `20260810171000_service_dispute_rpcs_gates_mmd.sql`
...
| ID | Item | Status |
|----|------|--------|
| ~~SC-01~~ | Disputa de serviço (`IN_DISPUTE`) | **Fechada (2026-08-10)** — ver glossário / CONTEXT decisão 33 |
| ~~SC-02~~ | Aba Disputas em Meus Serviços | **Fechada (2026-08-10)** — `list_phase = dispute` |

Fora do MVP: painel `/admin/*`, reopen self-serve, `IN_DISPUTE → CANCELLED`, reembolso automático, notificação a admins.

---

## 20. Atualização de auditoria

- **2026-08-10 (Disputa de serviço MVP)** — Stub de URL externa retirado do glossário de produto. Status `IN_DISPUTE`; RPCs open/admin resolve; gates cancel/auto-complete/confirm; `list_phase=dispute`; hold Ganhos `service_dispute`; SC-01/SC-02 fechadas. ADR-0006; runbook admin resolve em `service-completion-monitoring.md`.
- **2026-08-04** — Documentação de negócio alinhada ao cutover service-completion (READY-handoff, RPCs `service_completion_*`, UX enrichment/EXECUTED/confirm/auto-complete; disputa era stub — supersedido 2026-08-10).
...
- **2026-08-05 (UX)** — Checklist/avaliação saem do inline do detalhe: CTAs no detalhe → sheet (mobile) / dialog (desktop); cliente com stepper 2 etapas; fotos de evidência como thumbnails + lightbox. *(Host atual desde 2026-08-11: `ServiceDetailActionsBar`.)*
...
- **2026-08-06 (stub disputa — só no wizard)** — `ClientEvaluateServiceAction` deixa de renderizar `DisputeStubEntry` inline no detalhe quando não há CTA Avaliar serviço. Stub **nunca** no host do detalhe; permanece **somente** dentro do `ClientConfirmRatingWizard` no fluxo Avaliar serviço.
...
- **2026-08-07 (invalidação lista/detalhe nos hooks de mutação)** — `useProviderMarkExecuted` e `useClientConfirmRating` passam a invalidar, no `onSuccess`, além do contexto de conclusão, `SERVICES_LIST_QUERY_KEY` (`["view-services","list"]`) e `SERVICE_DETAIL_QUERY_KEY` (`["view-services","detail"]`). Assim Meus Serviços e o detalhe refrescam após mark-executed ou confirmação/avaliação a partir de **qualquer** host (card da lista, prompt global, CTA do detalhe). Removida invalidação quebrada em `ProviderExecutedWizard` com chave `["services"]` (não correspondia à chave real da lista).
