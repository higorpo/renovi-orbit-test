...
| 17 | [provider-earnings](./provider-earnings/README.md) | Ganhos do prestador: página unificada (Cobranças = captura; Depósitos = liquidações previstas/liquidadas) | `/dashboard/settings/earnings` (hub; sem menu; sem item Recebimentos na nav) | `src/features/provider-earnings/` | Concluída |
...
- **provider-earnings** → `/dashboard/settings/earnings` (hub Configurações, ledger Cobranças + Depósitos); disclosure também em `ProviderSettlementStatus` (payments).
