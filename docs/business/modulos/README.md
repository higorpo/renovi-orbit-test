...
| 16 | [provider-kyc](./provider-kyc/README.md) | Gate do conteúdo até `ACTIVE`; telas de status; menu completo intacto; wizard de credenciamento (Fase 3) | Embutido no `DashboardLayout` (exceção `/dashboard/conta*`) | `src/features/provider-kyc/` | Concluída (gate Fase 2 + wizard Fase 3) |
