...
| 4 | [my-account](./my-account/README.md) | Hub Minha conta (settings) | `/dashboard/account/*` | `src/features/my-account/` | Concluída |
...
- **provider-earnings** → `/dashboard/account/earnings` (hub Minha conta); disclosure também em payments.
- **provider-kyc** → gate de conteúdo + ocultação do chrome de nav no **dashboard-shell** até NetCred `ACTIVE`; allowlist **my-account**; backend KYC/cobrança em **payments**; lembretes incompletos via **message-dispatcher** (cron SQL).
...
1. ~~**`/dashboard/addresses`** placeholder~~ — **fechado:** rota/menu removidos; gestão em `/dashboard/account/addresses` e no wizard.
...
6. **Menu vs rotas:** Conversas no menu; Ganhos e calendário e detalhe de serviço são rotas reais **fora** do menu top-level (Ganhos no hub Minha conta); Visão geral / Ajuda / Configurações ainda placeholder.
