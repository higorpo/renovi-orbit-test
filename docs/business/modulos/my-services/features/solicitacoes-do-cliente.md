...
Prioridade (comprovada em `clientServiceCardPresentation` / `providerServiceCardPresentation`): **follow-up de conclusão vence unread** nos ramos críticos — cliente `EXECUTED` (avaliar) e prestador `CONFIRMED`+past / `EXECUTED` — no mesmo padrão de `FAILED_PERMANENT` vs unread. No **cliente**, `CONFIRMED`+past (“Aguardando conclusão…”) ainda pode ser sobrescrito por unread; nos demais `PENDING_PAYMENT` (não permanente), unread sobrescreve pagamento. Sem follow-up/unread/pagamento prioritário → highlight de agenda (`getScheduleHighlightContent`: “Agendado para…”, “Serviço hoje”, etc.).
