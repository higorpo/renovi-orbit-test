...
16. **Prioridade do highlight e CTAs `in_progress` (vs unread):** o fluxo de conclusão **vence** mensagem não lida, no mesmo espírito de `FAILED_PERMANENT` (pagamento). **Cliente — highlight:** (1) `FAILED_PERMANENT` → alerta de pagamento; (2) `EXECUTED` → “Aceite a conclusão e avalie o serviço”; (3) unread → nova mensagem; (4) `CONFIRMED` + past → “Aguardando conclusão do prestador”; (5) agenda. Nos demais `PENDING_PAYMENT` (não permanente), unread ainda sobrescreve o destaque de pagamento. **Cliente — CTA:** `EXECUTED` → “Avaliar serviço” **antes** de “Responder”; unread só vence quando não há `EXECUTED`/`FAILED_PERMANENT`. **Prestador — highlight e CTA:** follow-up de conclusão (`CONFIRMED` + past → “Marque…/Concluir serviço”; `EXECUTED` → “Aguardando confirmação…/Ver detalhes”) **vence** unread; só então unread, depois pagamento pendente / agenda.
...
| Concluir serviço (`mark_executed`) | `CONFIRMED` + timing `past` (banner follow-up; **prioridade sobre unread**) | Abre sheet/dialog no card (`ProviderMarkExecutedSheet` / `CompletionFlowSheetDialog` + `ProviderExecutedWizard` na fase `checklist`; após sucesso, fase `success` com `ProviderExecutedSuccessStep` → `CompletionSuccessStep`, `chrome="immersive"`); RPC de contexto só ao abrir; lista/detalhe refrescam via invalidação em `useProviderMarkExecuted` `onSuccess`; host **não** fecha a sheet | Disabled + tooltip se `!enrichmentReady` |
| Ver detalhes (follow-up `EXECUTED`) | `EXECUTED` (**prioridade sobre unread**) | Detalhe (aguardar confirmação / evidências no host) | — |
...
- `src/features/my-services/utils/clientServiceCardPresentation.ts`, `providerServiceCardPresentation.ts`, `clientServiceCardTheme.ts`, `providerProposalStatus.ts`; copy de pagamento pendente via `getPendingPaymentHighlightContent` (`view-services`)
...
Prioridade (comprovada em `resolveClientCardActions` / `resolveProviderCardActions` em `view-services`, consumidos por `clientServiceCardPresentation` / `providerServiceCardPresentation` e pelo card **Próximo passo** no detalhe): **follow-up de conclusão vence unread** nos ramos críticos — cliente `EXECUTED` (avaliar) e prestador `CONFIRMED`+past / `EXECUTED` — no mesmo padrão de `FAILED_PERMANENT` vs unread. No **cliente**, `CONFIRMED`+past (“Aguardando conclusão…”) ainda pode ser sobrescrito por unread; nos demais `PENDING_PAYMENT` (não permanente), unread sobrescreve pagamento. Sem follow-up/unread/pagamento prioritário → highlight de agenda (`getScheduleHighlightContent`: “Agendado para…”, “Serviço hoje”, etc.). O detalhe reusa o **mesmo ranking de intent primário** para o card Próximo passo (só intents acionáveis; ver glossário).
