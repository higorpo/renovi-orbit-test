...
- Destaque `PENDING_PAYMENT`: copy compartilhada em `view-services` (`getPendingPaymentHighlightContent`); no cliente, `FAILED_PERMANENT` prevalece sobre unread.
- Destaque de **follow-up de conclusão** (fase `in_progress`): quando a data fim agendada já passou (`getScheduledTiming` = `past`, end default = start) com contrato `CONFIRMED`, ou quando o contrato já está `EXECUTED`, o card deixa de mostrar só “Agendado para…” e passa a pedir ação de conclusão (copy por papel). **Prioridade vs unread:** cliente `EXECUTED` e prestador `CONFIRMED`+past / `EXECUTED` **vencem** unread (mesmo padrão de `FAILED_PERMANENT`); no cliente, `CONFIRMED`+past ainda pode ser sobrescrito por unread; nos demais `PENDING_PAYMENT` (não permanente), unread sobrescreve pagamento. Sem prefetch de `get_service_completion_context` na lista. **Prestador** `CONFIRMED` + past: primário **“Concluir serviço”** abre o checklist no card; após envio bem-sucedido, step de sucesso na mesma sheet (host invalida queries e **não** fecha); secundário “Ver detalhes”; botão disabled + tooltip se `!enrichmentReady`. **Cliente** `EXECUTED`: primário **“Avaliar serviço”** (`evaluate_service`) abre sheet/dialog na página (`ClientEvaluateServiceSheet` via `ClientEvaluateServiceDialogs` + `useClientEvaluateServiceDialog`); secundário “Ver detalhes”; contexto RPC só ao abrir o wizard. Prestador `EXECUTED`: primário “Ver detalhes” (aguardar no detalhe).
- **Cliente — card `completed`:** se `contracted.status === COMPLETED` e `clientRatingOverallScore == null` (já no modelo de `list_services` — sem request extra), primário **“Avaliar serviço”** (`evaluate_service`); secundário “Ver detalhes”; o host passa `ratingOnly` (só etapa de notas). Se já houver rating, mantém só “Ver detalhes”.
