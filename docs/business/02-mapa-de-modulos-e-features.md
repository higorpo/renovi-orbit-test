...
| **provider-kyc** | [gate-e-acesso-operacional](./modulos/provider-kyc/features/gate-e-acesso-operacional.md), [formulario-credenciamento-wizard](./modulos/provider-kyc/features/formulario-credenciamento-wizard.md) | Bloqueia **conteúdo** até `ACTIVE`; menu completo; exceção `/dashboard/conta*`; wizard entity→identity→bank→documents→review | `dashboard-shell`, `my-account`, `payments` (NetCred / cobrança) |
