...
| **my-services** | [solicitacoes-do-cliente](./modulos/my-services/features/solicitacoes-do-cliente.md) | `/dashboard/services`; banner prestador → calendário; highlights de pagamento e follow-up de conclusão no card | `view-services` (lista/detalhe); sheet de orçamentos via `negotiation-proposals`; `provider-calendar` (entrada); nudge conclusão → detalhe (`service-completion`) |
