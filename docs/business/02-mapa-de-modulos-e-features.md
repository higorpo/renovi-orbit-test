...
| **settings** | [configuracoes](./modulos/settings/features/configuracoes.md) | Hub `/dashboard/settings/*` (mobile lista + stack; desktop sidebar); ADR 0002; seção **Jurídico** (`legal`) — docs oficiais (ambos os papéis); prestador: **Ganhos** unificado (`earnings`; `?view=charges` = Cobranças; `?period=3m`/`6m`); rota legado `receivables` redireciona | `addresses`, storage, perfis público/privado; histórico de captura via `payments`; host de Ganhos (`provider-earnings`); site jurídico (`VITE_MAIN_SITE_URL`) |
