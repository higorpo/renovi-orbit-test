...
| **provider-kyc** | [gate-e-acesso-operacional](./modulos/provider-kyc/features/gate-e-acesso-operacional.md), [formulario-credenciamento-wizard](./modulos/provider-kyc/features/formulario-credenciamento-wizard.md), [lembretes-credenciamento-incompleto](./modulos/provider-kyc/features/lembretes-credenciamento-incompleto.md) | Bloqueia **conteúdo** até `ACTIVE`; **oculta chrome de nav** (`useProviderKycBlocksNav`); exceção `/dashboard/conta*`; wizard entity→identity→bank→documents→review; lembretes MMD para incompleto/`REJECTED` | `dashboard-shell`, `my-account`, `payments` (NetCred / cobrança), `message-dispatcher` |
