...
| **provider-earnings** | [ganhos-e-liquidacoes](./modulos/provider-earnings/features/ganhos-e-liquidacoes.md) | `/dashboard/settings/earnings` — página unificada (ledger Cobranças + Depósitos) hospedada no hub Configurações; **sem** item no menu top-level; **sem** item Recebimentos na nav | `payments` (settlements + lista de captura); `settings` (host); `dashboard-shell` (chrome) |
