...
|------------------------------------|---------------------|--------------------------|--------------------------------|
| **addresses** | [gestao-de-enderecos](./modulos/addresses/features/gestao-de-enderecos.md) | Embarcado em `request-quote` e hub `my-account` (`/dashboard/account/addresses`); rota top-level `/dashboard/addresses` **removida** | `auth` (usuário), Supabase `client_addresses`, geografia |
...
| **my-account** | [minha-conta](./modulos/my-account/features/minha-conta.md) | Hub `/dashboard/account/*` (mobile lista + stack; desktop sidebar); ADR 0002 | `addresses`, storage, perfis público/privado; histórico de captura via `payments`; host de Ganhos (`provider-earnings`) |
...
| **payments** | [checkout-e-cobranca](./modulos/payments/features/checkout-e-cobranca.md), [historico-e-reembolso](./modulos/payments/features/historico-e-reembolso.md), [reconciliacao-e-voids](./modulos/payments/features/reconciliacao-e-voids.md) | Checkout pós-aceite; histórico no hub `/dashboard/account/payments` \| `receivables`; ops/reconcile sem UI | `negotiation-proposals`, `my-account`, `provider-kyc`, `provider-earnings`, `service-reschedule`, NetCred EFs, RPCs `payment_*` (incl. opções de parcela + `min_installment_value`) |
| **provider-earnings** | [ganhos-e-liquidacoes](./modulos/provider-earnings/features/ganhos-e-liquidacoes.md) | `/dashboard/account/earnings` — UI hospedada no hub Minha conta; **sem** item no menu | `payments` (settlements); `my-account` (host); `dashboard-shell` (chrome) |
| **provider-kyc** | [gate-e-acesso-operacional](./modulos/provider-kyc/features/gate-e-acesso-operacional.md), [formulario-credenciamento-wizard](./modulos/provider-kyc/features/formulario-credenciamento-wizard.md), [lembretes-credenciamento-incompleto](./modulos/provider-kyc/features/lembretes-credenciamento-incompleto.md) | Bloqueia **conteúdo** até `ACTIVE`; **oculta chrome de nav** (`useProviderKycBlocksNav`); exceção `/dashboard/account*`; wizard entity→identity→bank→documents→review; lembretes MMD para incompleto/`REJECTED` | `dashboard-shell`, `my-account`, `payments` (NetCred / cobrança), `message-dispatcher` |
...
> **Não é placeholder:** `/dashboard/services/:id` → `ServiceDetailShell`; hub `/dashboard/account/*` (my-account + host de earnings). Rotas **removidas** (sem redirect): `/dashboard/conta`, `/dashboard/earnings`, `/dashboard/addresses`.

## Menu do dashboard (evidência: `dashboardMenu.ts`)

| Papel | Itens (`allItems`) |
|-------|--------------------|
| **Cliente** | Visão geral, Meus Serviços, Conversas, Minha conta (`/dashboard/account`), Ajuda |
| **Prestador** | Visão geral, Meus Serviços, Trabalhos, Conversas, Minha conta (`/dashboard/account`), Ajuda |

**Fora do menu (rotas reais):** `/dashboard/services/calendar`, `/dashboard/services/:id`, `/dashboard/settings`, seções `/dashboard/account/*` (incl. earnings).
