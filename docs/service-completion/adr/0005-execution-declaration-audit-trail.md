...
**Collection:** Capacitor `@capacitor/device` (+ web `userAgent` fallback). Approximate location comes from **IP geolocation on the Edge** (`ipwho.is`) — no GPS permission. Geo lookup failure stores `ip_geo` null and does not block the declaration.
