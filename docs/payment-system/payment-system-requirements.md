...
- If rescheduling updates `scheduled_start_date` / `scheduled_shift` and `payment_schedules.state ∈ {SCHEDULED, FAILED, IN_ANALYSIS}`: `charge_scheduled_at = MAX(now(), payment_service_execution_at(cs) − interval '2 days')`. Auto-cancellation T-12h uses the **updated** `payment_service_execution_at`. If `state = PAID` and new execution is ≤ `paid_at + far_reschedule_recapture_threshold_days`: only slot columns are updated; no new charge. If `state = PAID` and new execution is **more than** `paid_at + far_reschedule_recapture_threshold_days`: mark far-recapture pending and (via backend EF) refund fully then create a new `SCHEDULED` cycle at T-2.
...
**WHEN** the service is rescheduled and the schedule is `PAID` with new execution ≤ `paid_at + far_reschedule_recapture_threshold_days`  
...
**WHEN** the service is rescheduled and the schedule is `PAID` with new execution more than `paid_at + far_reschedule_recapture_threshold_days`  
