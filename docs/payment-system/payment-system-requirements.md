...
**THEN** `customerInput.persist` MUST be `false`; `customerInput.companyId` MUST be set to `netcred_company_id` of the service provider; `billingAddressInput` MUST be included in all production calls.
...
**THEN** the request body MUST include: `proposal_id`, `scheduled_date`, `payment_method` (`'CREDIT_CARD'`), `payment_token_id` (Renovi UUID from `payment_tokens`), `installment_number` (integer 1–12), and `installment_selection_hmac` (the signed payload from the calculation Edge Function); all fields MUST be required for the `CREDIT_CARD` payment method.
...
**WHEN** the final charge amount is computed  
**THEN** the cron MUST invoke the Postgres RPC `calculate_charge_amount(payment_token_id, base_amount, installment_number)` to compute `final_amount` using current `platform_constants`; this `final_amount` MUST be used in `chargeCreate.amount`; a previously computed or cached amount MUST NOT be used.
