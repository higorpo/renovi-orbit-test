...
const META_ROW_CLASS =
  "flex min-w-0 items-start gap-1.5 text-sm font-normal leading-snug text-muted-foreground";
...
export function JobCard({ job, className, onDismiss, isDismissing = false }: JobCardProps) {
  ...
  return (
    <Card
      className={cn(
        "flex w-full max-w-full min-w-0 flex-col overflow-hidden rounded-xl border bg-card p-0 shadow-sm transition-[box-shadow,border-color] duration-150 hover:border-border hover:shadow-md",
        className,
      )}
    >
      <div className="flex w-full min-w-0 flex-col gap-3 p-4">
        <div className="flex min-w-0 items-start gap-2">
          <div className="min-w-0 flex-1 space-y-2">
            ...
            {presentation.showFallbackBadge || urgencyBadge ? (
        ...
        <button
          type="button"
          className="flex w-full min-w-0 flex-col gap-2.5 rounded-lg py-0.5 text-left text-sm font-normal transition-colors hover:bg-muted/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 max-sm:ring-offset-0"
          onClick={handleOpenDetails}
          aria-label={`Ver detalhes: ${job.title}`}
        >
          <div className="flex min-w-0 flex-col gap-1">
            <h2 className="line-clamp-2 min-w-0 text-base font-semibold leading-snug text-foreground">
              {job.title}
            </h2>
            {presentation.description ? (
              <p className="line-clamp-2 min-w-0 break-words text-sm leading-snug text-foreground/75">
                {presentation.description}
              </p>
            ) : null}
          </div>
          ...
          <div className="flex min-w-0 flex-col gap-1">
            <p className={META_ROW_CLASS}>
              ...
              <span className="min-w-0 break-words">{presentation.locationLine}</span>
            </p>
            <p className={META_ROW_CLASS}>
              <Clock className="mt-0.5 h-3.5 w-3.5 shrink-0" aria-hidden />
              <span className="min-w-0 break-words">{presentation.publishedLine}</span>
            </p>
          </div>
        </button>
      </div>
