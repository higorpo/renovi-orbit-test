...
describe("useProviderMarkExecutedDialog", () => {
  it("opens and closes with the selected service model", () => {
    const model = serviceModel();
    const { result } = renderHook(() => useProviderMarkExecutedDialog(), { wrapper });

    act(() => result.current.openMarkExecuted(model));
  ...
  it("invalidates service queries after executed", () => {
    ...
    const { result } = renderHook(() => useProviderMarkExecutedDialog(), {
