...
} from "../types/clientSignup.validation";
...
export function useClientSignupForm() {
    ...
    try {
      ...
      const redirectTo =
        typeof window !== "undefined"
          ? `${window.location.origin}${CLIENT_ONBOARDING_PATH}`
          : undefined;
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setSubmitting(true);
    const token = await executeRecaptcha("client_signup_submit");
    if (!token) {
    ...
    const recaptchaCheck = await verifyRecaptchaToken(token, "client_signup_submit");
