...
import { getAccountNavItems } from "../constants/accountNav";
import { ROUTE_ACCOUNT_PERSONAL_INFO } from "../constants/routes";
 ...
 * Desktop: redirect to personal-info (sidebar lives in MyAccountLayout).
 */
export function MyAccountIndexPage() {
  ...
  if (isDesktop) {
    return <Navigate to={ROUTE_ACCOUNT_PERSONAL_INFO} replace />;
  }

  return (
    <div className="container max-w-lg px-4 py-6">
      <h1 className="mb-6 text-2xl font-bold tracking-tight text-foreground">Minha conta</h1>
      <div className="mb-6">
        <MobileAccountSummary role={role} email={user?.email ?? ""} />
      </div>
      <AccountNavList items={items} variant="list" />
    </div>
  );
}
