-- CNS Phase 12 — task 81: 1MB list RPC response cap with payload projection (design §9.5, R22-AC04).
-- Updates list_conversations (task 58) and list_chat_messages (task 59).

create or replace function public.cns_list_response_max_bytes()
returns integer
language sql
immutable
...
comment on function public.cns_list_response_max_bytes() is
  'Maximum JSON body size for list_conversations / list_chat_messages responses (1 MiB, R22-AC04).';
...
comment on function public.cns_project_message_payload_for_list(public.cns_message_type, jsonb) is
  'List projection: omits heavy fields; full payload via get_proposal_for_timeline / Realtime (task 81).';

create or replace function public.cns_assert_list_response_size(p_body jsonb)
returns jsonb
language plpgsql
immutable
set search_path = public
as $$
declare
  v_max int := public.cns_list_response_max_bytes();
  v_size int;
begin
  v_size := octet_length(p_body::text);

  if v_size > v_max then
    raise exception 'LIST_PAYLOAD_TOO_LARGE'
      using
        errcode = 'P0001',
        detail = jsonb_build_object(
          'code', 'LIST_PAYLOAD_TOO_LARGE',
          'max_bytes', v_max,
          'actual_bytes', v_size,
          'hint', 'Reduce p_page_size / p_limit or use cursor pagination'
        )::text;
  end if;
...
comment on function public.cns_assert_list_response_size(jsonb) is
  'Rejects list RPC responses over 1 MiB after projection (maps to HTTP 413 at API layer, R22-AC04).';
...
comment on function public.list_chat_messages(uuid, integer, timestamptz, uuid, boolean) is
  'Participant message history with list payload projection and 1 MiB response cap (R22-AC04, task 81).';
...
comment on function public.list_conversations(integer, timestamptz, uuid) is
  'Participant inbox with 1 MiB response cap; last-message preview only (no full payload, task 81).';
