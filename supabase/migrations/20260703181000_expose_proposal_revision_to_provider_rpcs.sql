-- Expose client revision request category and notes in provider-facing RPC payloads.
-- Based on current local definitions of get_provider_proposal_job_detail and
-- list_provider_proposal_history (queried via pg_get_functiondef).
...
comment on function public.list_provider_proposal_history(uuid) is
  'Provider proposal history for a service request, including pricing and revision request fields.';
