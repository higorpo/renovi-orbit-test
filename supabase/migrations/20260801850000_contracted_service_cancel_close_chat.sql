-- Close contracted-service chat and insert SYSTEM message on cancellation (CNS + payments).

alter type public.cns_closure_type add value if not exists 'CONTRACTED_SERVICE_CANCELLED';

comment on type public.cns_closure_type is
  'Why a chat was closed: manual, competitor accept, SR cancel, proposal reject, contracted service cancel.';
...
begin
  ...
  if p_initiator = 'provider' then
    return
      ...
      || '(o processamento pode levar de 30 a 60 dias; taxas de cartão não são reembolsadas).';
  end if;
  ...
  return case coalesce(p_refund_tier, '')
    when 'FULL_REFUND' then
      ...
      || '(taxas de cartão não são reembolsadas).'
...
create or replace function public.cns_close_contracted_service_chat(
...
begin
  ...
  v_message := public.cns_build_contracted_service_cancel_system_message(
    p_initiator,
    p_refund_tier,
    p_pre_charge,
  ...
  with closed as (
    ...
    set
      ...
      closure_type = 'CONTRACTED_SERVICE_CANCELLED'::public.cns_closure_type,
      closed_at = now(),
      closed_by_user_id = p_closed_by_user_id,
      closure_reason = left(v_message, 2000),
      updated_at = now()
  ...
  select
    ...
    jsonb_build_object('text', v_message),
    'service_request',
    v_service.service_request_id,
    gen_random_uuid()
  from closed;
...
comment on function public.cns_close_contracted_service_chat(uuid, uuid, text, text, text, boolean) is
  'Closes the contracted provider chat and inserts a cancellation SYSTEM message (idempotent when already closed).';

revoke all on function public.cns_build_contracted_service_cancel_system_message(text, text, boolean, text)
  from public, anon, authenticated;
revoke all on function public.cns_close_contracted_service_chat(uuid, uuid, text, text, text, boolean)
  from public, anon, authenticated;

grant execute on function public.cns_build_contracted_service_cancel_system_message(text, text, boolean, text)
  to service_role;
grant execute on function public.cns_close_contracted_service_chat(uuid, uuid, text, text, text, boolean)
  to service_role;
...
comment on function public.cns_service_request_allows_chat_messaging(uuid, uuid) is
  'Messaging gate: OPEN SR or active (non-cancelled) contracted service on the winning provider chat.';
