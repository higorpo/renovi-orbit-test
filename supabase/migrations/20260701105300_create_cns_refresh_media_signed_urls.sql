-- CNS Phase 7 — task 56: refresh authorized chat-media paths for signed URL renewal (design §5.1, Req. 31, R31-AC06).
-- Migration order: runs AFTER tasks 4, 15.
-- Postgres has no storage.sign; RPC authorizes paths and returns bucket + expires_in for client createSignedUrl.
...
comment on function public.cns_refresh_media_signed_urls(uuid[], text[], int) is
  'Participant-only refresh gate for chat-media IMAGE paths (max 20). Returns bucket, expires_in, and authorized paths for client createSignedUrl (R31-AC06).';

revoke all on function public.cns_refresh_media_signed_urls(uuid[], text[], int) from public;
revoke all on function public.cns_refresh_media_signed_urls(uuid[], text[], int) from anon;

grant execute on function public.cns_refresh_media_signed_urls(uuid[], text[], int) to authenticated;
grant execute on function public.cns_refresh_media_signed_urls(uuid[], text[], int) to service_role;
