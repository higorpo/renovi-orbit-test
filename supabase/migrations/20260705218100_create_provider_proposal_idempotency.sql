-- Idempotency for create_provider_proposal (operation chats.submit_proposal, design §3.10).

drop function if exists public.create_provider_proposal(
  uuid,
  numeric,
  text,
  integer,
...
comment on function public.create_provider_proposal(
  ...
  'Provider proposal create/resubmit with rpc_idempotency_records replay (operation chats.submit_proposal).';
