-- CNS Wave E — task 36: client SR cancel cascade RPC (design §4.4, Req. 2, 11).
-- Migration order: runs AFTER tasks 13, 23–24, 3, 35.
...
begin
  ...
  -- Reject proposals before SR cancel so the legacy cancel trigger does not race this RPC.
  ...
  set
    ...
    client_rejection_response = coalesce(
      ...
      'Proposta recusada automaticamente: pedido cancelado pelo cliente.'
    )
...
comment on function public.cancel_service_request(uuid, uuid) is
  'Atomic client cancel: SR CANCELLED, bulk chat close, non-terminal proposals REJECTED_AUTOMATICALLY (R2-AC05, R2-AC06, OAC-11).';
