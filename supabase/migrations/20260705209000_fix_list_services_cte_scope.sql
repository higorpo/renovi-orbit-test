-- Fix list_services: CTE scope only applies to the immediately following statement in PL/pgSQL.
...
begin
  ...
  select
    ...
    coalesce(
      (
        select jsonb_agg(public.project_service_row(p.id, v_viewer_id) order by sr.updated_at desc)
        from page_ids p
        join public.service_requests sr on sr.id = p.id
      ),
      '[]'::jsonb
    )
