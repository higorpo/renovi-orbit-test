-- Client republishes a cancelled service request as a new OPEN row (matching bootstrap via INSERT trigger).
...
comment on function public.republish_cancelled_service_request(uuid, uuid) is
  'Client duplicates a cancelled service request into a new OPEN row; reuses photo paths and triggers matching dispatch bootstrap.';

revoke all on function public.republish_cancelled_service_request(uuid, uuid) from public;
revoke all on function public.republish_cancelled_service_request(uuid, uuid) from anon;
grant execute on function public.republish_cancelled_service_request(uuid, uuid) to authenticated;
