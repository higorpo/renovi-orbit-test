-- Keep REJECTED / EXPIRED / REJECTED_AUTOMATICALLY terminal when provider submits a new proposal.
-- REVISED remains only for superseded REVISION_REQUESTED rows (revision flow).
-- Relax unique_active index to gate only in-flight proposals (PENDING, REVISION_REQUESTED).
...
comment on index public.provider_proposals_unique_active is
  'At most one in-flight proposal (PENDING or REVISION_REQUESTED) per provider per service request.';
