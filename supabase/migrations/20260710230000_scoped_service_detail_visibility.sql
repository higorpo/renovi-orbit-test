-- Scope service detail payload by viewer: client-only proposal totals, contracted
-- summary only for client or winning provider, omit tags from API responses.
-- get_service and list_services both delegate to project_service_row.

create or replace function public.provider_sees_full_service_address(
...
comment on function public.provider_sees_full_service_address(uuid, uuid) is
  'True when the provider has an ACCEPTED proposal on the service request (full address visibility gate).';

revoke all on function public.provider_sees_full_service_address(uuid, uuid) from public;
revoke all on function public.provider_sees_full_service_address(uuid, uuid) from anon, authenticated;
...
comment on function public.project_service_row(uuid, uuid) is
  'Builds unified service JSON. Client-only proposal totals; contracted summary for client or accepted provider; tags omitted; provider address masked until ACCEPTED.';
