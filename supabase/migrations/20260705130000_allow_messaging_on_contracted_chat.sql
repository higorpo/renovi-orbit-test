-- Allow messaging in the contracted provider chat after accept (SR COMPLETED).
...
comment on function public.cns_service_request_allows_chat_messaging(uuid, uuid) is
  'OPEN SR allows any participant chat; COMPLETED SR allows only the contracted provider chat.';

revoke all on function public.cns_service_request_allows_chat_messaging(uuid, uuid) from public;
grant execute on function public.cns_service_request_allows_chat_messaging(uuid, uuid) to authenticated;
grant execute on function public.cns_service_request_allows_chat_messaging(uuid, uuid) to service_role;
