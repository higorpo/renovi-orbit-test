...
begin
  if v_actor is null then
    raise exception 'Authentication required for submit_proposal'
      using errcode = '42501';
  end if;
  ...
  if v_actor <> v_chat.provider_id then
    raise exception 'Only the provider may submit a proposal'
      using errcode = '42501';
  end if;
  ...
  if found then
    ...
    where pp.provider_id = v_actor
      and pp.service_request_id = v_chat.service_request_id
  ...
  v_cached := jsonb_build_object(
    ...
    'timeline_message', jsonb_build_object(
      'id', v_message.id,
...
comment on function public.submit_proposal(
  uuid,
  uuid,
  numeric,
  text,
  ...
  'Provider proposal submit: pricing validation, PENDING insert, PROPOSAL timeline message, PROPOSAL_SUBMITTED outbox (R6-AC01, R32-AC03).';
