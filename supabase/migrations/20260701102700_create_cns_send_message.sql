-- CNS Wave B — task 28: primary message ingress RPC (design §4.1–4.2, Req. 1, 3, 4, 29, 34).
-- Migration order: runs AFTER tasks 2–4, 3, 23–26.
...
begin
  ...
  if p_chat_id is not null then
    ...
    -- Lock service_requests before chats (same order as the p_service_request_id path).
    select *
    into v_sr
...
comment on function public.cns_send_message(
  ...
  'Primary CNS message ingress: chat create/reactivate, slot gate, free-messaging gate, rate limit, outbox CHAT_MESSAGE_SENT (R1-AC01, R3-AC01, OAC-02).';
