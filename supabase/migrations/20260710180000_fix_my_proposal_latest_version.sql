-- Fix my_proposal in project_service_row: pick latest submitted proposal by version,
-- not by updated_at (expiration/rejection bumps updated_at on older rows).
...
comment on function public.project_service_row(uuid, uuid) is
  'Builds unified service JSON. Provider my_proposal uses highest version (excludes superseded REVISED rows).';
