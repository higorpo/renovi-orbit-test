-- Expose service address coordinates in project_service_row when full address is visible.
...
comment on function public.project_service_row(uuid, uuid) is
  'Builds unified service JSON. Client-only proposal totals; contracted summary (with chat_id) for client or accepted provider; tags omitted; provider address masked until ACCEPTED; coordinates only when full address is visible.';
