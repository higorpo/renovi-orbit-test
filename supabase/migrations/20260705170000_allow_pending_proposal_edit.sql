-- Allow provider to replace an in-flight PENDING proposal (edit flow).
-- Prior PENDING row becomes REJECTED_AUTOMATICALLY; new row is inserted as PENDING.
...
comment on function public.create_provider_proposal(
  ...
  'Canonical proposal creation by service_request_id; PENDING edits supersede prior PENDING as REJECTED_AUTOMATICALLY; REVISED only supersedes REVISION_REQUESTED.';
