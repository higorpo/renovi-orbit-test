-- Expose contracted provider chat_id in project_service_row (get_service / list_services).
...
comment on function public.project_service_row(uuid, uuid) is
  'Builds unified service JSON. Client-only proposal totals; contracted summary (with chat_id) for client or accepted provider; tags omitted; provider address masked until ACCEPTED.';
