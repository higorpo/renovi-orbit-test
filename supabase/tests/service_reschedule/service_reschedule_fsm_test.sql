-- pgTAP: service reschedule FSM core (request → propose → accept).
...
begin
  ...
  select
    ...
    format('service reschedule fsm %s', v_service_id),
  ...
  perform set_config('test.fsm_service_id', v_service_id::text, true);
  perform set_config('test.fsm_client_id', v_client_id::text, true);
  perform set_config('test.fsm_provider_id', v_provider_id::text, true);
end;
$seed$;

select pg_temp.cns_set_auth(current_setting('test.fsm_client_id')::uuid);

create temp table _fsm_request as
select public.cns_request_service_reschedule(
  current_setting('test.fsm_service_id')::uuid,
  gen_random_uuid(),
  null
) as response;

select is(
  (select response->'reschedule'->'active_request'->>'status' from _fsm_request),
  'REQUESTED',
  'request opens REQUESTED active request'
);

select pg_temp.cns_set_auth(current_setting('test.fsm_provider_id')::uuid);

create temp table _fsm_propose as
...
select is(
  (select response->'reschedule'->'active_request'->>'status' from _fsm_propose),
  'PROPOSED',
  'propose moves request to PROPOSED'
);

select pg_temp.cns_set_auth(current_setting('test.fsm_client_id')::uuid);

create temp table _fsm_accept as
...
select is(
  (
    ...
    where cs.id = current_setting('test.fsm_service_id')::uuid
  ),
  'afternoon',
