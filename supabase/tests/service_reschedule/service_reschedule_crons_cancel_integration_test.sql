...
select public.expire_stale_service_reschedule_requests(10) as payload;
...
select ok(
  (select (payload->>'expired_count')::int >= 1 from _expire_result),
  'expiration janitor reports at least one expired request'
);
