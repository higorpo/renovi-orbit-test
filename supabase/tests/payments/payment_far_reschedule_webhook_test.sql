-- pgTAP: TRANSACTION_REFUND webhook on far-recapture path must not cancel CS/chat.
...
begin
  ...
  from public.calculate_provider_service_pricing(900.00::numeric);
  ...
  select v_sr, sr.client_id, sr.service_id, sr.address_id, 'far webhook pgTAP',
  ...
  insert into public.provider_proposals (
    ...
    v_proposal, v_provider_id, v_sr, v_pricing.original_amount, 'far webhook', 1, 'days',
  ...
  insert into public.payment_schedules (
    ...
    v_cs, v_client_id, v_provider_id, 1, 900.00, 15.00, 765.00,
    now() - interval '2 days', 'PAID', v_cs::text, v_cs,
    now() - interval '1 day', 920.00, 'txn-far-webhook',
    'FAR_RESCHEDULE_RECAPTURE'
  )
  returning id into v_schedule_id;

  perform set_config('test.far_wh.cs', v_cs::text, true);
  perform set_config('test.far_wh.schedule', v_schedule_id::text, true);
  perform set_config('test.far_wh.chat', v_chat_id::text, true);
  perform set_config('test.far_wh.event', v_event_id::text, true);
end;
$seed$;

select is(
  (
    select public.payment_webhook_handle_refund(
      current_setting('test.far_wh.event')::uuid,
      jsonb_build_object(
        'referenceCode', current_setting('test.far_wh.cs'),
        'refundedAmount', '920.00'
      )
    )->>'outcome'
  ),
  'refunded',
  'webhook refund applies on far-recapture PAID schedule'
);
...
select is(
  (
    ...
    where cs.id = current_setting('test.far_wh.cs')::uuid
  ),
  'CONFIRMED',
  'webhook far-recapture does not cancel contracted service'
);
...
select is(
  (
    ...
    from public.chats c
    where c.id = current_setting('test.far_wh.chat')::uuid
  ),
  'ACTIVE',
  'webhook far-recapture does not close chat'
);
...
select is(
  (
    ...
    where ps.id = current_setting('test.far_wh.schedule')::uuid
  ),
  'REFUNDED',
  'webhook moves far-recapture schedule to REFUNDED'
);
