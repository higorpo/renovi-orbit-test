...
begin
  ...
  -- Boundary: 14 days → near (strict > threshold for far)
  v_sr := gen_random_uuid();
  v_proposal := gen_random_uuid();
...
select is(
  ...
  'PAID + ≤15d → paid_no_charge_update'
);
...
select is(
  ...
  'PAID + >15d → paid_far_recapture_required'
);
...
select is(
  ...
  'paid_no_charge_update',
  'boundary at 14 days remains paid_no_charge_update'
);
